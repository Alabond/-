...
# 说明: 本文件在 F4 场景下承担“候选子系统生成与着色筛选”职责。
# 主要入口:
#   - FindF4_B4_Subsystems(custom_roots, custom_colors)
#     按指定位置标签、黑白约束扫描候选 simple-root 组并输出标准记录。
# 关键子函数用途:
#   - F4_InnerProduct / F4_Reflection:
#       在 simple-root 系数坐标下计算内积与简单反射。
#   - F4_IsRootBlack:
#       按 F4(4) 约定判定非紧根（黑点）与紧根（白点）。
#   - F4_IsValidSimpleCoeffRootVec:
#       校验根向量是否落在可接受系数边界与符号一致性范围内。
#   - BuildSelectedIndices / BuildStates:
#       将标签配置转成索引组合，并通过 Weyl 反射扩展状态空间。
#   - BuildRecord:
#       构造统一子系统记录，填充 roots/colors/type/components 等字段。
#############################################################################
